...
persistent w0_wr_frac;
    if(isempty(w1_wr_frac)&& isempty(w0_wr_frac))
        w1_wr_frac=zeros(256,18);  % 64*4
        w0_wr_frac=zeros(256,18);
    end
 % BRAM_int
...
for u = 1:17
if isempty(flag)
    p_out_plus = zeros(1,18); p_out_minus = zeros(1,18); 
end

    %y1y0 == '10'; 
%different BRAM from digitreuse coz "x_ori" is only read in digitreuse but both "write" and "read" in control and CA_gen 

[u_r,wr_addr,rd_addr,enable,add_enable,res_enable,x1_value,x0_value,y1_value,y0_value] = control_v2(xin1(u),xin0(u),y1(u),y0(u),n,u);
...
if isempty(flag)
     cin_one1 = 0; cin_one0 = 0; cin_two1 = 0; cin_two0 = 0;CAw_frac1=zeros(1,18);CAw_frac0=zeros(1,18);
else
    % BRAM_frac read
...
flag =1;
                ...
                p_out_plus(1,u)=p_value1;
                p_out_minus(1,u)=p_value0;
                p1_out=p_out_plus;
                p0_out=p_out_minus;
end
end
